@php
    ...
    $js = ['bank'];
@endphp
@extends('layouts.app', ['title' => _lang('Update Training')])
@section('page.header')
	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">{{ _lang('Update Training') }}</span>
			</div>
...
<div class="card border-top-success rounded-top-0" id="table_card">
	<div class="card-header header-elements-inline bg-light border-grey-300" >
		<h5 class="card-title">{{ _lang('Update Training Information') }}
		...
		<form class="form-validate-jquery" id="content_form"  action="{{ route($route . 'update', $model->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-12 form-group">
                    <label for="picture">Background Picture</label>
                    @if ($model->picture)
                        <input type="file" name="picture" id="picture" class="form-control dropify" data-default-file="{{ asset('storage/training/'. $model->picture) }}">
                    @else 
                ...
                <div class="col-md-4 form-group">
                    ...
                    <input type="text" value="{{ date('m-d-Y', strtotime($model->event_date)) }}" name="date" id="" class="form-control date" value="{{ date('m-d-Y') }}">
                </div>

                <div class="col-md-4 form-group">
                    <label for="start_time">Start Time </label>
                    <input type="text" value="{{ $model->start_time }}" name="start_time" id="start_time" class="form-control" placeholder="Enter Start Time">
                </div>

                <div class="col-md-4 form-group">
                    <label for="end_time">End Time </label>
                    <input type="text" value="{{ $model->end_time }}" name="end_time" id="end_time" class="form-control" placeholder="Enter End Time">
                </div>
                
                <div class="col-md-12 form-group">
                    <label for="content">Content</label>
                    <textarea name="description" id="content" cols="30" rows="4" class="form-control">{{ $model->description }}</textarea>
                </div>
