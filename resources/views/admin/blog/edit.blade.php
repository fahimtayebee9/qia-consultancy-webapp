@php
    ...
    $js = ['bank'];
@endphp
@extends('layouts.app', ['title' => _lang('Update Blog')])
@section('page.header')
	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">{{ _lang('Update Blog') }}</span>
			</div>
            ...
            <div class="row">
                ...
                <div class="col-md-4 form-group">
                    ...
                    <select name="author_id" id="author_id" class="form-control select" data-placeholder="Select Author" required data-parsley-errors-container="#sub_category_id_error">
                        ...
                        @foreach ($authors as $author)
                            <option {{ $model->author_id == $author->id ? 'selected' : '' }} value="{{ $author->id }}">{{ $author->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12 form-group">
                    ...
                    <select name="tags[]" id="tags" multiple class="form-control select" data-placeholder="Select Tag">
                        ...
                        @php
                            $tags_array = explode(',', $model->tags);
                        @endphp
                        @foreach ($tags as $tag)
                            <option {{ in_array($tag->id, $tags_array) ? 'selected' : '' }} value="{{ $tag->id }}">{{ $tag->name }}</option>
                        @endforeach
                    </select>
                </div>
